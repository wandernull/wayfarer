...
- **Routing**: two static HTML files. `/` serves `public/index.html` (the marketing landing page — story-telling + pricing + SEO meta). `/plan` serves `public/plan.html` (the SPA / form / itinerary / paywall / payment flow). Shareable journey URLs `/journey/<uuid>` rewrite to `/plan` (clean URL, NOT `/plan.html` — Pages auto-301s `.html` → clean form, and that 301 fires inside the rewrite if the destination still has `.html`, kicking the user out to a real navigation). The rewrite is `200` (URL bar stays as `/journey/<uuid>`), and the SPA inspects `location.pathname` on boot. Landing CTAs (`<a href="/plan">`) are plain links — no JS modal, the form lives at its own URL.
...
| GET  | `/api/journey/[id]` | Full record (payment block sanitized; includes `payment.invoice = { id, number, hostedUrl, pdfUrl }` once paid — those URLs are pre-signed by Stripe and safe to expose). 404 if expired. |
| GET  | `/api/journey/[id]/status` | Lightweight `{ status, paidAt }`. |
| GET  | `/api/locale` | Returns `{ country, language, currency, amount, display }` resolved from the client's IP-based country (Cloudflare's `request.cf.country`). TR → `try / 24999 / "₺249,99"`; everything else → `eur / 499 / "€4.99"`. `language` is the suggested UI language (TR→tr, ES→es, FR→fr, DE→de, IT→it, PT→pt, else en). SPA + landing call this on boot to swap `{price}` tokens **and** auto-set the UI language on first visit. |
| POST | `/api/journey/[id]/buy` | Creates Stripe Checkout session, returns URL. Idempotent if already paid. Accepts `{ locale, productName, productDescription }` so the Stripe-hosted UI **and** the line item copy match the user's selected language. Locale is whitelisted; product strings are length-capped (60 / 500 chars) before being forwarded. Currency + unit_amount on the Checkout session come from the same `resolveCountry / priceForCountry` helpers as `/api/locale` — so the price the user saw on the SPA and the price Stripe charges always agree. |
...
13. **Geographic pricing**: TR-IP customers see and pay **₺249,99**; everyone else sees and pays **€4.99**. Resolution is purely IP-based (Cloudflare's `request.cf.country`) — locale (UI language) is independent. Decision was deliberate: indie founder testing TR market parity (~€4.71 at recent rates), VPN/billing-mismatch edge cases explicitly accepted as MVP-level noise. Both `/api/locale` and `/api/journey/[id]/buy` share the same `resolveCountry`/`priceForCountry` helpers so display ↔ checkout always agree. Copy uses a `{price}` token that `t()` substitutes at render time; the SPA + landing fetch `/api/locale` on boot and re-apply translations once the display value lands. Brief flicker (~50–100ms) on first paint for TR users — accepted. Adding more country/price tiers means just extending the `PRICES` object in `functions/api/locale.js`.
14. **Share affordance**: the bottom-bar Export PDF button was replaced 2026-05-10 with **↗ Share**. Behaviour branches on `window.matchMedia('(pointer: coarse)').matches`: touch devices fire the OS native share sheet via `navigator.share({ title, text, url })`; desktop opens a small dropdown with **Download PDF** (still `window.print()`) + **Copy URL** (`navigator.clipboard.writeText` with `execCommand` fallback) — desktop's native share dialog is awkward and gives no clear download/copy affordance, so we route around it. Click-outside closes the menu, copy success shows a 1.8 s toast.
...
Once the webhook lands paid, `bootFromUrl` shows a one-shot ✅ success modal (`.paid-modal-card`) with View invoice / Download PDF buttons (sourced from `payment.invoice.hostedUrl` / `pdfUrl` set by the webhook). The modal only appears when `?paid=1` is present in the URL, and `bootFromUrl` strips that query param immediately after, so the modal never re-shows on refresh, share-link visits, or subsequent loads. Two trigger paths: (a) poll landed paid → modal then strip; (b) webhook beat the SPA and `data.payment.status === 'paid'` already at boot → modal then strip. The webhook fetches the invoice via `GET /v1/invoices/<id>` after the session-completed event; failure is non-fatal (trip still flips to paid, modal renders without invoice buttons).
...
- **Tax**: Stripe Tax is enabled on every Checkout session via `automatic_tax[enabled]=true`, `billing_address_collection=auto`, `tax_behavior: inclusive`, `tax_code: txcd_10000000` (general digital service). `auto` lets Stripe minimise the address form — typically just country + postal code in the EU, ZIP only on US cards, nothing extra when the user pays via Apple/Google Pay (the wallet supplies the address). Inclusive means the customer-facing total stays €4.99 — VAT is carved out of that amount on the seller side (NL 21% → ~€4.12 net). Only one Stripe Tax registration is active today: **Domestic (Netherlands)**, since the company is a Dutch BV. Other-EU customers therefore pass through tax-free until **EU OSS** is added (see Open work). Tax IDs aren't collected (`customer_tax_id_collection` left off) — purely B2C product, no B2B reverse-charge flow. Sandbox mirrors the live setup separately — Tax registrations live on the *account* and don't carry over from live mode, so the same Domestic NL row has to be added in sandbox too for test charges to compute VAT.
- **Invoices**: every Checkout session auto-creates a proper invoice via `invoice_creation[enabled]=true`. Stripe assigns a sequential invoice number (required for Dutch BTW), generates a PDF with the VAT breakdown, and emails the customer in their `locale`. Line item name + description are the localized strings forwarded from the SPA. Invoice footer points to `support@jounee.app` for corrections. View / re-send any invoice from Stripe dashboard → Invoices. **Email delivery requires** Settings → Customer emails → "Email finalized invoices and credit notes" turned on per-account (live + sandbox separately).
- **B2B / tax-ID collection**: `tax_id_collection[enabled]=true` adds an "I'm purchasing as a business" toggle on Checkout. Customers enter company name + VAT/Tax ID; Stripe validates EU VAT IDs against VIES. Triggers EU intra-community reverse-charge (0% VAT, recipient self-accounts) — works without OSS since the B2B exemption is independent of home registration. NL→NL B2B keeps 21% BTW (no intra-NL reverse-charge); the ID just lands on the invoice for the buyer's records.

## Open work

### Worth doing soon

- **Register for EU OSS at Belastingdienst, then add the OSS registration in Stripe Tax.** Today only NL→NL sales are taxed; other-EU customers pass through at 0%, which is technically non-compliant for a Dutch BV under the €10k cross-border threshold (should still charge 21% NL VAT) and above it (must charge destination rates via OSS). OSS registration takes ~10 working days at Belastingdienst → Mijn Belastingdienst Zakelijk → "Aanmelden voor de Unieregeling". Once active, add the OSS row in Stripe Tax → Registrations and Stripe will auto-apply the right rate per EU country. No code change needed — `automatic_tax[enabled]=true` already wired.
...
- ~~**Multi-city header rendering.**~~ Resolved 2026-05-08. The itinerary main panel now renders a prominent hero with the parent destination(s) (e.g. "Paris" or "Paris &amp; Rome", DM Serif Display) plus Claude's `tripTitle` as italic subtitle. `document.title` also reflects the destination so the browser tab and shared-link previews aren't generic. The header pill keeps the sub-area route as secondary reference.
- **PDF export** is currently `window.print()` — invoked from the new "↗ Share" button's "Download PDF" menu item. Fine for MVP; a server-side branded PDF is the obvious next step.
...
- **Orphan code**: `buildSummary`, `selectBudget`, `calcLatestEndTime`, `nights_unit`/`sb_nights` translation keys are unused after the form rework but kept for safety. Delete in a focused cleanup commit. (The orphan `share` translation key was repurposed into `share_trip` on 2026-05-10.)
